...
# Overview
## Sample
Below is a sample analysis for a moral dilution study, with a total sample of *N* = `r length(x$gender)`, (female = `r sum(x$gender=="2")`, male = `r sum(x$gender=="1")`, non-binary/other = `r sum(x$gender=="3") + sum(x$gender=="5")`, prefer not to say `r sum(x$gender=="5")`, *M~age~* = `r round(mean(x$age),digits=1)`, *SD* = `r round(sd(x$age),digits=1)`).
...
#summary(lm(R_tot~condition,data = x))
#summary(aov(R_tot~condition,data = x))
...
## Testing for differences in non-diagnostic conditions
